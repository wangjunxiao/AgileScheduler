...
import cn.dlut.util.CoordinatorMessageUtil;
...
public class ControllerChannelHandler extends OFChannelHandler {
    ...
    enum ChannelState {
        ...
        WAIT_FT_REQ(false) {
            ...
            void processOFFeaturesRequest(final ControllerChannelHandler h,
                ...
                if (reply == null) {
                    h.log.error("VirtualSwitch failed to return a featuresReply message: {}"
                            + h.sw.getSwitchName());
                    h.channel.disconnect();
                }
        ...
        protected void unhandledMessageReceived(
            ...
            if (m.getType() == OFType.VENDOR) {
                h.log.warn(
                        ...
                        m);
                OFMessage e = CoordinatorMessageUtil.makeErrorMsg(
    ...
    public ControllerChannelHandler(final VirtualSwitch sw) {
        ...
        if (e.getMessage() instanceof List) {
            ...
            for (final OFMessage ofm : msglist) {
                ...
                try {
                	...
                	switch(ofm.getType()) {
                	case STATS_REQUEST:
                		System.out.println("switch receive "+((OFStatisticsRequest)ofm).
                				getStatisticType()+"from"+ctx.getChannel().getRemoteAddress());
					default:
						break;
                	}*/
                    switch (ofm.getType()) {
                    case PACKET_OUT:
                         ...
                         * then send it to the Virtual Network.
                         */
                        final byte[] data = ((OFPacketOut) ofm).getPacketData();
                        if (data.length >= 14) {
                            if (CoordinatorLLDP.isLLDP(data)) {
                            	//VirtualSwitch handle LLDP      
                                ((VirtualSwitch)this.sw).handleLLDP(ofm);
                                break;
                            }
                        }
                    default:
